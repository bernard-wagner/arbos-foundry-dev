...
use foundry_evm_core::constants::DEFAULT_STYLUS_DEPLOYER;
use revm::{
    bytecode::Bytecode,
...
fn deploy_stylus_code(
    ...
    let deploy_calldata = deployCall::new((
        init_code.into(),
    ...
    let deployer_address = ccx
        ...
        .stylus_config
        .deployer_address
        .unwrap_or(DEFAULT_STYLUS_DEPLOYER);
    ...
    // Record caller balance before to compute actual activation fee afterwards
    let balance_before = ccx.ecx.journaled_state.load_account(caller)?.data.info.balance;

    // Load deployer account into journal (exec_call bypasses pre-execution account loading)
    ccx.ecx.journaled_state.load_account(deployer_address)?;
    ...
    // Compute actual activation data fee from the caller's balance change.
    // Net cost to caller = init_value + data_fee (budget minus refund).
    let balance_after = ccx.ecx.journaled_state.load_account(caller)?.data.info.balance;
    ...
    if is_broadcasting {
        if let Some(last_tx) = ccx.state.broadcastable_transactions.back_mut() {
            if let Some(tx) = last_tx.transaction.as_unsigned_mut() {
